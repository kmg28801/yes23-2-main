...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDevlieryCancelled_then_UPDATE_2(@Payload DevlieryCancelled devlieryCancelled) {
        try {
            if (devlieryCancelled.isMe()) {
                // view 객체 조회
                List<Mypage> mypageList = mypageRepository.findByOrderId(devlieryCancelled.getOrderId());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenPayConfirmed_then_UPDATE_3(@Payload PayConfirmed payConfirmed) {
        try {
    ...
    public void whenPayCancelled_then_UPDATE_4(@Payload PayCancelled payCancelled) {
        try {
            if (payCancelled.isMe()) {
                ...
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mypage.setStatus(payCancelled.getStatus());
                    // view 레파지 토리에 save
                    mypageRepository.save(mypage);
                }
            }
